	...
	<div class="slidercard" :style="customStyle">
		...
		<div class="container">
			<div class="card">
				...
				<wwLayoutColumn tag="div" ww-default="ww-text" :ww-list="section.data.titles" class="content card-title" @ww-add="add(section.data.titles, $event)" @ww-remove="remove(section.data.titles, $event)">
		...
		slide(direction) {
			if (!this.editMode) {
				if (direction === 'right' && this.canSwipeRight) {
					if (this.colsPosition < 100 * (this.featuresLength - 1)) this.colsPosition += 100 / this.featuresLength
				} else if (direction === 'right' && !this.canSwipeRight) {
					this.colsPosition = 0
				} else {
					if (this.colsPosition > 0) this.colsPosition -= 100 / this.featuresLength
				}
